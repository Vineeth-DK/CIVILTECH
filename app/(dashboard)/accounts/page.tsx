...
export default function AccountsPage() {
  const { projects, currentFilter, searchQuery, dateFilter, completeAccounts } = useProjectStore();
  const [selectedProject, setSelectedProject] = useState<Project | null>(null);
  const [isModalOpen, setIsModalOpen] = useState(false);

  const allTouched = projects.filter((p) => p.stages.accounts.status !== 'pending');
  const pending    = allTouched.filter((p) => p.stages.accounts.status === 'in_progress').length;
  const completed  = allTouched.filter((p) => p.stages.accounts.status === 'completed').length;

  const displayed = useMemo(
    () => applyFilters(projects, 'accounts', currentFilter, searchQuery, dateFilter, false),
    [projects, currentFilter, searchQuery, dateFilter],
  );

  const stats = [
    { label: 'Total',          value: allTouched.length, icon: <DollarSign className="w-5 h-5 text-sky-500 dark:text-sky-400" />, color: 'bg-sky-500/10' },
    { label: 'Pending',        value: pending,            icon: <Clock className="w-5 h-5 text-indigo-500 dark:text-indigo-400" />, color: 'bg-indigo-500/10' },
  ...
  return (
    <div className="flex flex-col h-full">
      <Topbar title="Accounts" subtitle="Billing, invoicing & project closure" />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
              <AnimatePresence mode="popLayout">
                {displayed.map((project, i) => (
                  <ProjectCard key={project.id} project={project} stage="accounts"
                    onAction={(p) => { setSelectedProject(p); setIsModalOpen(true); }}
                    actionLabel="Close Project →" index={i} />
                ))}
              </AnimatePresence>
            </div>
          )}
      ...
      <GenericActionModal project={selectedProject} isOpen={isModalOpen}
        ...
        title="Close Project" description="Mark project fully invoiced and archived. This is the final pipeline step."
        actionLabel="Close & Archive" onConfirm={completeAccounts} variant="success" accentColor="sky" />
    </div>
  );
}
