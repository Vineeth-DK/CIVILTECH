...
import { Layers, Activity, CheckCircle2, BarChart2 } from 'lucide-react';
...
export default function MappingPage() {
  const { projects, currentFilter, searchQuery, dateFilter, completeMapping } = useProjectStore();
  const [selectedProject, setSelectedProject] = useState<Project | null>(null);
  const [isModalOpen, setIsModalOpen] = useState(false);

  const allTouched = projects.filter((p) => p.stages.mapping.status !== 'pending' && p.stages.mapping.status !== 'bypassed');
  const pending    = allTouched.filter((p) => p.stages.mapping.status === 'in_progress').length;
  const completed  = allTouched.filter((p) => p.stages.mapping.status === 'completed').length;

  const displayed = useMemo(
    () => applyFilters(projects, 'mapping', currentFilter, searchQuery, dateFilter, true),
    [projects, currentFilter, searchQuery, dateFilter],
  );

  const stats = [
    { label: 'Total',     value: allTouched.length, icon: <Layers className="w-5 h-5 text-emerald-500 dark:text-emerald-400" />, color: 'bg-emerald-500/10' },
  ...
  return (
    <div className="flex flex-col h-full">
      <Topbar title="Mapping" subtitle="Topographic & site mapping" />
      ...
      <GenericActionModal project={selectedProject} isOpen={isModalOpen}
        ...
        title="Complete Mapping" description="Mark mapping complete. Project routes to Drafting."
